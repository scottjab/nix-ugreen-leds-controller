...
in
{
  options.services.ugreen-leds = {
    ...
    diskMonitor = {
      ...
      mappingMethod = mkOption {
        type = types.enum [ "ata" "hctl" "serial" ];
        default = "ata";
  ...
  config = mkIf cfg.enable (let
    ...
    configFileContent = ''
      ...
      ${optionalString (cfg.networkMonitor.colorLink100 != null) ''COLOR_NETDEV_LINK_100="${formatColor cfg.networkMonitor.colorLink100}"''}
      ${optionalString (cfg.networkMonitor.colorLink1000 != null) ''COLOR_NETDEV_LINK_1000="${formatColor cfg.networkMonitor.colorLink1000}"''}
      ${optionalString (cfg.networkMonitor.colorLink2000 != null) ''COLOR_NETDEV_LINK_2000="${formatColor cfg.networkMonitor.colorLink2000}"''}
      ${optionalString (cfg.networkMonitor.colorLink2500 != null) ''COLOR_NETDEV_LINK_2500="${formatColor cfg.networkMonitor.colorLink2500}"''}
      ${optionalString (cfg.networkMonitor.colorLink5000 != null) ''COLOR_NETDEV_LINK_5000="${formatColor cfg.networkMonitor.colorLink5000}"''}
      ${optionalString (cfg.networkMonitor.colorLink10000 != null) ''COLOR_NETDEV_LINK_10000="${formatColor cfg.networkMonitor.colorLink10000}"''}
      ...
      CHECK_GATEWAY_CONNECTIVITY=${if cfg.networkMonitor.checkGatewayConnectivity then "true" else "false"}
    ...
    systemd.services = mkMerge [
      ...
      (mkIf cfg.diskMonitor.enable {
        ugreen-diskiomon = {
          ...
          wantedBy = [ "multi-user.target" ];
          restartTriggers = [ package configFileContent ];
        };
      })
