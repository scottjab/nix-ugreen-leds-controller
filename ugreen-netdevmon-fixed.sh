...
COLOR_NETDEV_LINK_PURPLE_DEFAULT=${COLOR_NETDEV_LINK_PURPLE_DEFAULT:="128 0 128"} 
...
function set_netdev_normal_color() {
    ...
    if [[ $CHECK_LINK_SPEED_DYNAMIC == true ]]; then
        ...
        case $(cat /sys/class/net/$netdev_name/speed) in
            ...
            2000)  color=${COLOR_NETDEV_LINK_2000:=$COLOR_NETDEV_LINK_PURPLE_DEFAULT};;
            2500)  color=${COLOR_NETDEV_LINK_2500:=$COLOR_NETDEV_NORMAL};;
            5000)  color=${COLOR_NETDEV_LINK_5000:=${COLOR_NETDEV_LINK_10000:=$COLOR_NETDEV_LINK_PURPLE_DEFAULT}};;
            10000) color=${COLOR_NETDEV_LINK_5000:=${COLOR_NETDEV_LINK_10000:=$COLOR_NETDEV_LINK_PURPLE_DEFAULT}};;
        esac
    fi
